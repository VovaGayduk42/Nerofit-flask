...
from model_udb import *
from sqlAlchemy_insert import user_add
...
def home():
    if not session.get('logged_in'):
        ...
        global USER_ID
        print("identif"+str(db_session.query(User.id).first()))
...
def do_admin_login():
   ...
   # print('identificator' + str(user.id))
    query = db_session.query(User).filter(User.username.in_([POST_USERNAME]), User.password.in_([POST_PASSWORD]))
    result = query.first()



    print('query tag=', query)
...
@app.route('/home')
def home_render():
    return render_template('pages/lk/home.html')
